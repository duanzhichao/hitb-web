    ...
    <el-row :gutter="20" style="margin-top: 120px;">
      ...
      <el-col :span="8">
        <el-card class="box-card">
          <div slot="header">
            <span style="text-align: center;color: #0082ff;font-weight: bold;">新用户注册</span>
          </div>
          ...
          <div>
            <div>用户名<span style="font-size: 6px;">(用户名长度大于4小于16位,并包含数字或字母)</span></div>
          ...
          <div style="margin-top: 5px;">
            <div>密码<span style="font-size: 6px;">(密码长度不小于6位且必须含有英文字母)</span></div>
...
export default {
  ...
  methods: {
    ...
    singUp () {
      ...
      if (!regexpUsername.test(this.user.username)) {
        this.user.error = true
        this.user.errorLog = '请检查您的用户名'
        this.captchaSuccess = false
        return;
      }
      // 邮箱验证
      ...
      const data = { drg_admin_user: { username: this.user.username, password: this.user.password, confirm: this.user.repassword, email: this.user.email, captcha: this.user.captcha } }
      apiRequest(this, 'post', '/servers/drg_admin_user', data).then(res => {
        if (!res.is_success) {
          this.user.error = true
